...
## Game State
The authoritative record of all facts about the current game world: player stats, inventory, location, NPC relationships, quest status, session history. Stored persistently by the Engine on disk. The LLM never holds Game State in its own memory — it receives it fresh on every turn.

## Turn
One complete player interaction cycle. Two variants:
- **Turn ohne Probe:** Spieler-Input → Kontext-Assembly → LLM-Narration (mit Engine-Tools) → State-Update.
- **Turn mit Probe:** Spieler-Input → DM fordert Probe an (Probe-Anforderung: Skill + Difficulty Tier) → Engine fordert Würfelwurf an → Spieler gibt W20-Ergebnis ein (zweiter Input) → Engine berechnet Endergebnis inkl. Ticks → LLM-Narration des berechneten Ergebnisses → State-Update. Der Spieler würfelt physisch; das Engine-Ergebnis wird nie intern gewürfelt wenn eine Probe nötig ist.

## Probe-Anforderung
The mandatory path for every action with an uncertain outcome: the DM LLM names a Skill from the Skill List and a Difficulty Tier; the Engine maps the tier to a SG, blocks for the player's physical W20, and computes the result (including crits, ticks, damage). The LLM never resolves an uncertain action in prose alone — doing so is a Rule Bypass. A rule-based Validator checks each narration against the Game State afterwards. Supersedes the earlier Action Classifier (a separate pre-narration LLM call) — see ADR-0001.
...
## Combat State
The `combat` object inside the character's Game State, present only while a fight is running. It holds the round number, the enemy list, a pending roll, whether the PC has already acted this round, and any Active Defense. Combat begins when the Narrator calls `start_combat`; a second `start_combat` during a running fight adds reinforcements rather than failing. Combat ends by itself the moment no able enemy is left — `end_combat` is only for a fight that stops without a winner (the PC flees, both sides talk). Leaving this to the Narrator meant a finished fight stayed in the save, blocked every new one, and left old and new enemies mixed together. Initiative is situational (ambush, who strikes first) — there is no initiative roll. The Narrator receives a `=== KAMPFZUSTAND ===` context block each turn listing every enemy with current HP, distance, status, and whether they have acted this round.
...
## Combat Round
One full cycle in which the PC and every able enemy acts exactly once. The Engine advances the round by itself — there is no turn-ending tool. When the PC and all able enemies have acted, the Engine starts the next round: melee enemies advance one Distance Zone, a dying PC bleeds, and any Active Defense expires. An enemy still closing the distance counts as occupied for that round. One player message is one round: at the end of the turn the Engine closes the round unconditionally, and an enemy the Narrator never ran through `npc_action` simply forfeits its action. Without that hard close a single forgotten enemy action wedged the fight forever — every later player action was rejected with "already acted this round".

## State Fingerprint
Nine numbers taken before a turn and compared after it: HP, total copper, location slug, clock in minutes, combat presence, combat-log length, summed enemy HP, enemy count, injury count. The Validator uses the delta to check *claims* the narration makes — money changing hands, a blow landing, time passing, a scene changing. Checking tool names instead was unsound: the server recorded them before execution, so a `pay` that failed on insufficient funds still satisfied the coin rule. Prohibitions ("no XP or Verteidigungswert in the prose") stay text checks, because those ask what the text says, not what happened. See ADR-0006.

## Name Register
The eighth context layer: one line per canonical proper name in range of the current location — name, type, slug, role, faction, and any World Flag set during this Playthrough. Scope is the Region downward, so it stays a few thousand characters even in a dense city. It exists because the earlier layers only ever carried *present* NPCs in full text: the moment the Narrator talked about someone who was elsewhere, it had no canon for them at all and reinvented their office (a City Watch captain became a harbor master mid-conversation). The register is the boundary between drift and invention — what is listed is settled, what is missing does not exist yet and has to go through `add_wiki_entry` first. See ADR-0004.
...
## Narrator Output
Everything the DM LLM changes about the world flows through typed Engine tools during narration: time advancement, coin and inventory movements, new World-Scoped locations, new NPCs, World State Flag updates, combat control, and Probe-Anforderungen. Prose alone never changes Game State. Each tool call is validated by the Engine before it is applied; an invalid call is rejected with an error the LLM must correct. (Historical note: an earlier design returned one structured JSON document per turn — superseded by the tool loop, see ADR-0001.)
...
## Playthrough
A single character's run through the world. All Character-Scoped content (generated minor NPCs, World State Flags, relationship data, journal, session synopses) belongs to exactly one Playthrough. The World Scope itself (the wiki: locations, major NPCs, lore) persists and grows across all Playthroughs — a new Playthrough sees the canonical world without any previous character's Flags. Deleting a Playthrough removes only Character-Scoped data.
...
API keys live server-side in the local `.env` (the game server runs on the player's own machine and keys never leave it). Model selection is a game setting changeable in the UI. The character wizard contains no API configuration.

## World State Flag
A Character-Scoped value that records how a World-Scoped entity has changed during a Playthrough (e.g. `tavern_burned=true`, `warehouse_owner=killed`). The Context Builder checks these flags and dynamically overrides or extends the static World Layer text when assembling the prompt. During play the DM changes existing world entities only through Flags — direct edits to the World Scope are reserved for authoring (the world editor). Flags do not carry over into a new Playthrough. See ADR-0002.
